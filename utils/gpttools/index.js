...
const fs = require('fs');
const payload = fs.readFileSync(__dirname + '/init.txt').toString(); // gpt init file to establish foods, recipes and the dataformat command 
...
OpenAPI.command = async (content) => { // helper command to easily command gpt
    let cmd = { role: "user", content: payload + ' ' + content };
